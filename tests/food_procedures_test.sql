...
DECLARE @u_sara_log INT = (SELECT user_id FROM core.users WHERE username = 'sara_ahmadi');
DECLARE @count_before_log INT = (SELECT COUNT(*) FROM food.food_logs);
PRINT 'scenario 1: full log entry';
EXEC food.sp_write_log
    @actor_id = @u_sara_log, @operation_type = 'insert', @target_table = 'test_table',
...
DECLARE @u_sara_cart INT = (SELECT user_id FROM core.users WHERE username = 'sara_ahmadi');
...
EXEC food.sp_clear_cart @user_id = @u_sara_cart;
PRINT 'scenario 1: quantity 0';
EXEC food.sp_add_to_cart @user_id = @u_sara_cart, @branch_id = @branch1_cart, @menu_item_id = @mi_b1_kabab_cart, @quantity = 0;
SELECT COUNT(*) AS row_count_after_scenario1 FROM food.cart_items WHERE user_id = @u_sara_cart;
PRINT 'scenario 2: valid new item, quantity 2';
EXEC food.sp_add_to_cart @user_id = @u_sara_cart, @branch_id = @branch1_cart, @menu_item_id = @mi_b1_kabab_cart, @quantity = 2;
SELECT * FROM food.cart_items WHERE user_id = @u_sara_cart;
PRINT 'scenario 3: same item added again with quantity 1';
EXEC food.sp_add_to_cart @user_id = @u_sara_cart, @branch_id = @branch1_cart, @menu_item_id = @mi_b1_kabab_cart, @quantity = 1;
SELECT * FROM food.cart_items WHERE user_id = @u_sara_cart;
PRINT 'scenario 4: unavailable menu item';
EXEC food.sp_add_to_cart @user_id = @u_sara_cart, @branch_id = @unavailable_branch_id, @menu_item_id = @mi_b1_unavailable, @quantity = 1;
SELECT COUNT(*) AS row_count_after_scenario4 FROM food.cart_items WHERE user_id = @u_sara_cart;
PRINT 'scenario 5: different branch while cart has branch1 items';
EXEC food.sp_add_to_cart @user_id = @u_sara_cart, @branch_id = @branch3_cart, @menu_item_id = @mi_b3_pizza_cart, @quantity = 1;
SELECT branch_id, COUNT(*) AS item_count FROM food.cart_items WHERE user_id = @u_sara_cart GROUP BY branch_id;

GO

PRINT 'sp_clear_cart';
DECLARE @u_sara_clear INT = (SELECT user_id FROM core.users WHERE username = 'sara_ahmadi');
...
EXEC food.sp_clear_cart @user_id = @u_sara_clear, @branch_id = @branch1_clear;
SELECT COUNT(*) AS remaining_rows FROM food.cart_items WHERE user_id = @u_sara_clear AND branch_id = @branch1_clear;
PRINT 'scenario 2: clear entire cart';
EXEC food.sp_add_to_cart @user_id = @u_sara_clear, @branch_id = @branch1_clear, @menu_item_id = @avail_mi_b1, @quantity = 1;
EXEC food.sp_clear_cart @user_id = @u_sara_clear;
SELECT COUNT(*) AS remaining_rows FROM food.cart_items WHERE user_id = @u_sara_clear;
...
DECLARE @u_new_staff_test INT = (SELECT user_id FROM core.users WHERE username = 'sara_ahmadi');
...
DECLARE @u_existing_owner INT = (SELECT user_id FROM food.branch_owners WHERE branch_id = @branch5_staff);
...
DECLARE @order_pending_cancel INT = (SELECT order_id FROM food.food_orders WHERE status = 'pending');
